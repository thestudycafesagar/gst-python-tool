...
class BulkMailApp(tk.Tk):
    ...
    def _build_header(self):
        ...
        settings_btn = ttk.Button(inner, text="⚙ Email Settings", style="Import.TButton", command=self._open_email_settings)
        settings_btn.pack(side="right", padx=(14, 0))

        link_lbl = tk.Label(inner, text="How to setup email 🔗", bg=HDR_BG, fg="#4da8da", font=("Segoe UI", 10, "underline"), cursor="hand2")
        link_lbl.pack(side="right", padx=(14, 14))
        link_lbl.bind("<Button-1>", lambda e: webbrowser.open("https://www.youtube.com/watch?v=MkLX85XU5rU"))
    ...
    def _open_email_settings(self):
        ...
        # Card styling for popup
        top.geometry("440x280")
        title_fr = tk.Frame(top, bg=BG)
        title_fr.pack(fill="x", pady=(20, 10))
        tk.Label(title_fr, text="⚙", font=("Segoe UI Emoji", 16), bg=BG, fg="#4da8da").pack(side="left", padx=(30, 10))
        tk.Label(title_fr, text="Gmail Details", font=("Segoe UI", 14, "bold"), bg=BG, fg=TEXT).pack(side="left")
        
        import tkinter.ttk as ttk
        frame = tk.Frame(top, bg=CARD, padx=20, pady=20, highlightbackground=BORDER, highlightthickness=1)
        frame.pack(padx=25, fill="x")
        
        tk.Label(frame, text="Gmail Address:", bg=CARD, fg=TEXT, font=("Segoe UI", 10, "bold")).grid(row=0, column=0, sticky="w", pady=5)
        e_email = ttk.Entry(frame, width=28)
        e_email.grid(row=0, column=1, sticky="w", pady=5, padx=10)
        e_email.insert(0, email_val)
        
        tk.Label(frame, text="App Password:", bg=CARD, fg=TEXT, font=("Segoe UI", 10, "bold")).grid(row=1, column=0, sticky="w", pady=(15, 5))
        e_pass = ttk.Entry(frame, width=28, show="*")
    ...
    def _rebuild_recipients_tab(self):
        ...
        ttk.Button(toolbar, text="�  Download Sample",
                   style="Import.TButton",
                   command=self._download_sample).pack(side="left", padx=(8, 0))
