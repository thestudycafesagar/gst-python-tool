...
class CustomTemplateStore:
    ...
    def save_all(self):
        with open(self._path, 'w', encoding='utf-8') as f:
            _json.dump({'templates': [t.to_dict() for t in self._templates]}, f, indent=2, ensure_ascii=False)
...
# ═══════════════════════════════════════════════════════════════
#  CUSTOM EMAIL SEND ENGINE  (Gmail SMTP — any attachment type)
# ═══════════════════════════════════════════════════════════════

def send_custom_emails(template, recipients, attachment_folder, log_cb, done_cb):
    import smtplib, glob as _glob
    from email.mime.multipart import MIMEMultipart
    from email.mime.text      import MIMEText
    from email.mime.application import MIMEApplication
    ...
    if not sender_email or not app_password:
        log_cb("[ERROR] Gmail credentials not set. Click '⚙ Email Settings' and save your Gmail address and App Password.")
        done_cb(success=False)
        return
    ...
    try:
        ...
        log_cb(f"[ERROR] Cannot connect to Gmail SMTP: {exc}")
        done_cb(success=False)
        return
    ...
    for idx, row in enumerate(recipients, 1):
        ...
        try:
            ...
            msg["From"]    = sender_email
            msg["To"]      = email
            cc_email       = str(template.cc).strip()
            if cc_email:
                msg["Cc"]  = cc_email
            msg["Subject"] = template.build_subject(cfg, row)
            body_text      = template.build_body(cfg, row)
                ...
                if fname:
                    ...
                    if att_path:
                        with open(att_path, "rb") as att:
                            part = MIMEApplication(att.read(), Name=os.path.basename(att_path))
            ...
            all_recips = [email]
            if cc_email:
                all_recips.extend([r.strip() for r in cc_email.split(",") if r.strip()])
            server.send_message(msg, from_addr=sender_email, to_addrs=all_recips)
            log_cb(f"[SENT]  {name} <{email}>")
...
class CustomMailApp(ctk.CTk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Custom Email Builder — Gmail")
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(top, text="⚙ Email Settings", height=32,
                       command=self._open_email_settings).pack(side="right", padx=18)
    ...
    def _save_template(self):
        ...
        if not name:
            messagebox.showwarning("Name Required",
                "Please enter a template name.", parent=self)
            return
        t = CustomTemplate(
            name=name,
    ...
    def _delete_template(self):
        if not (0 <= self._active_idx < len(self._store.templates)):
            messagebox.showinfo("Nothing to Delete",
                "No saved template selected.", parent=self)
            return
        name = self._store.templates[self._active_idx].name
        if not messagebox.askyesno("Confirm Delete",
                f"Delete template '{name}'?", parent=self):
            return
    ...
    def _download_sample(self):
        ...
        if not cols:
            messagebox.showwarning("No Columns",
                "Apply the template to Recipients first.", parent=self)
            return
    ...
    def _open_email_settings(self):
        ...
        top = ctk.CTkToplevel(self)
        top.title("⚙ Gmail Account Settings")
        top.geometry("480x320")
        top.transient(self)
        top.grab_set()

        ctk.CTkLabel(top, text="⚙  Gmail Account Settings", font=F_BADGE).pack(pady=(22, 10))

        frame = ctk.CTkFrame(top, corner_radius=8, border_width=1)
        frame.pack(padx=26, fill="x", pady=8)

        ctk.CTkLabel(frame, text="Gmail Address:",
                      font=("Segoe UI", 10, "bold")).grid(
            row=0, column=0, sticky="w", pady=10, padx=14)
        e_email = ctk.CTkEntry(frame, width=260)
        e_email.grid(row=0, column=1, sticky="ew", pady=10, padx=14)
        e_email.insert(0, email_val)

        ctk.CTkLabel(frame, text="App Password:",
                      font=("Segoe UI", 10, "bold")).grid(
            row=1, column=0, sticky="w", pady=(14, 10), padx=14)
        e_pass = ctk.CTkEntry(frame, width=260, show="*")
        e_pass.grid(row=1, column=1, sticky="ew", pady=(14, 10), padx=14)
        e_pass.insert(0, pass_val)

        link = ctk.CTkLabel(top, text="How to get a Gmail App Password? 🔗",
                              font=("Segoe UI", 10, "underline"),
                              cursor="hand2", text_color="#4da8da")
        link.pack(pady=(0, 8))
        link.bind("<Button-1>",
                   lambda e: __import__("webbrowser").open_new_tab(DEMO_VIDEO_URL))

        def _save():
            with open(cred_path, "w") as f:
                _json.dump({"email": e_email.get().strip(),
                             "password": e_pass.get().strip()}, f)
            top.destroy()
            messagebox.showinfo("Saved", "Gmail settings saved successfully!", parent=self)

        ctk.CTkButton(top, text="Save Settings", command=_save).pack(pady=10)
        ...
        def _show():
            ...
            if success:
                ...
                messagebox.showerror("Send Error",
                    ...
                    "Check your Gmail address and App Password in ⚙ Email Settings.",
                    parent=self)
        self.after(0, _show)
